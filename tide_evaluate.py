#!/usr/bin/env python3
"""Dataset-independent answer generation and fixed-judge validation.

Input is JSONL. By default each row has:
  {"id": ..., "question": ..., "context": ..., "answers": [...]}

Field names can be changed with CLI arguments, including dotted paths. The
program never imports benchmark loaders or project-local modules. It saves raw
model generations, raw judge decisions, and aggregate exact-match/token-F1/
judge-accuracy metrics so every reported value remains auditable.
"""
...
REFUSAL = re.compile(
    r"\b(?:i do not know|i don't know|cannot answer|can't answer|not enough information|"
    r"insufficient information|unable to determine|unknown)\b",
    re.IGNORECASE,
)
META_OUTPUT = re.compile(
    r"\b(?:as an ai|reference answers?|model answer|system prompt|instructions?|"
    r"i was asked to|cannot comply)\b",
    re.IGNORECASE,
)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--answerable-field", default=None)
...
def deterministic_validity(question: str, prediction: str) -> Tuple[bool, List[str]]:
    """Reject structurally invalid answers without changing semantic judgments."""
    ...
    if META_OUTPUT.search(text):
        reasons.append("meta_or_prompt_output")
    if metric_normalize(text) == metric_normalize(question):
        reasons.append("question_copy")
    if text.endswith((",", ";", ":", "-", "/", "(", "[")):
...
def run_generation(args: argparse.Namespace, rows: List[Dict[str, Any]], output: Path) -> None:
    ...
    records = []
    for row, prompt, raw in zip(pending, prompts, predictions):
...
def run_validation(args: argparse.Namespace, generations: Path, output: Path) -> None:
    ...
    if pending:
        ...
        for row, raw in zip(pending, decisions):
            ...
            valid, validity_reasons = deterministic_validity(row["question"], row["prediction"])
            ...
            records.append({
                **row,
                ...
                "deterministic_reasons": validity_reasons,
                "final_correct": bool(judge_correct) and valid,
            })
        append_jsonl(output, records)
...
def summarize(validated: Path, summary: Path) -> None:
    ...
    answerable = [row for row in parsed if bool(row.get("answerable", bool(row.get("answers"))))]
    attempted = [
        row for row in answerable
        if str(row.get("prediction") or "").strip() and not REFUSAL.search(str(row.get("prediction") or ""))
    ]
    correct = [row for row in answerable if bool(row.get("final_correct"))]
    answer_precision = len(correct) / len(attempted) if attempted else 0.0
    answer_recall = len(correct) / len(answerable) if answerable else 0.0
    answer_f1 = (
        2 * answer_precision * answer_recall / (answer_precision + answer_recall)
        if answer_precision + answer_recall else 0.0
    )
    result = {
        "examples": len(rows),
        ...
        "attempted_answers": len(attempted),
